...
export default function useApiClient() {
    ...
    const getHeaders = () => {
        ...
        console.log("API Client: token used for request:", token); // <-- Add this
        if (token) {
            headers["Authorization"] = `Bearer ${token}`;
        }
        console.log("API Client: headers used for request:", headers); // <-- Add this
        return headers;
    };
        ...
        async get(endpoint: string) {
            try {
                const response = await fetch(`${baseURL}${endpoint}`, {
                    method: "GET",
                    headers: getHeaders(),
                    // Remove credentials: "include" if not using cookies for auth
                });
                if (!response.ok) {
